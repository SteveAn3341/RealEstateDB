...
from properties
         inner join propertydetails ON properties.PropertyID = propertydetails.PropertyID
where bedrooms = (select max(bedrooms) from propertydetails);


select *
from agents
         inner join properties using(agentid)
where listingprice in (select ListingPrice from properties where ListingPrice > 500000 );
...
select * from properties inner join propertydetails using(propertyid)
...
where AmenityName in (select AmenityName from propertyamenities where AmenityName = "pool");



select amenityname, count(AmenityName) as an
from properties
         inner join propertyamenityassignments using (propertyid)
         inner join propertyamenities using(amenityid)
...
from propertyamenities
         inner join propertyamenityassignments using (amenityid)
group by amenityid
having count(AmenityID) > 3;
...
WHERE c.clientid IN (
    ...
    WHERE p.bedrooms > 3 AND t.saleprice > 500000
);